...
- [ ] **FIRST CHECK — front-camera mirroring.** This is the highest-risk unknown in the app and cannot be settled off-device. `CameraService.configureVideoConnection()` pins the data-output connection to `automaticallyAdjustsVideoMirroring = false` / `isVideoMirrored = false`, so Vision sees raw sensor orientation and `CoordinateMapper(isMirrored:)` applies the flip exactly once. Verify: on the FRONT camera, raise your LEFT hand → the skeleton's raised hand must appear on the same side of the screen as your hand does. Then confirm a limb hint names the correct side ("adjust your left arm" when the left arm is wrong). Repeat on the rear camera. If the skeleton is flipped, the double-mirror is real: the fix is in `configureVideoConnection()` (or `CoordinateMapper.isMirrored`), NOT in the pose data.
- [ ] Live skeleton tracks body at arm's length and full-body distance; joints stable.
